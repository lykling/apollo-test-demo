...
int main(int argc, char** argv) {
  ...
  auto sender =
      node->CreateWriter<apollo::cyber::examples::cyberrt_test::proto::Frame>(
          "/apollo/msg2");
  apollo::cyber::ReaderConfig reader_config;
  reader_config.channel_name = "/apollo/msg";
  reader_config.pending_queue_size = 30;
  auto receiver =
      node->CreateReader<apollo::cyber::examples::cyberrt_test::proto::Frame>(
          reader_config,
          [sender,
           &statis](const std::shared_ptr<
                    apollo::cyber::examples::cyberrt_test::proto::Frame>& msg) {
            msg->set_read_timestamp(apollo::cyber::Time::Now().ToNanosecond());
            statis.count++;
            statis.total_time += msg->read_timestamp() - msg->write_timestamp();
            statis.transmit_time_list.emplace_back(msg->read_timestamp() -
                                                   msg->write_timestamp());
  ...
  double sum = std::accumulate(std::begin(statis.transmit_time_list),
                               std::end(statis.transmit_time_list), 0.0);
  double mean = sum / statis.transmit_time_list.size();
  double accum = 0.0;
  std::for_each(std::begin(statis.transmit_time_list),
                std::end(statis.transmit_time_list),
